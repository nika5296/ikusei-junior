 ...
 *        既存シート時: true（既定）で生徒行を作り直す（退会で名前列から消える）。
 *        false は背景のみ（日次トリガー用）
...
function applyLayout(sheet, year, month) {
  ...
  // ── 条件付き書式: 「振」＝薄黄色（スクリプト実行順に依存しないようシート側ルールも同期）
  ensureRosterMakeupConditionalFormat_(sheet, year, month);
}

/**
 * 日付列エリアに「セル値が 振 なら背景 #fff2cc」の条件付き書式を1本だけ置く
 * （applyLayout のたびに古い同一ルールを除去して差し替え）
 */
...
/** メンバーが以前同期した「振＝黄色」条件付き書式か（差し替え用に除去） */
...
function applyBackground(sheet, year, month) {
  ...
  // 全学生 × 全日付 の背景色を配列で一括構築し、1回のAPI呼び出しで設定
  // 休会中の生徒は全セルをグレー（在籍はしているが出席不可）
  // ※既に「振」が入っているセルはレギュラー外でも黄色を維持（振替出席の見た目を統一）
...
function updateFromLog() {
  ...
  Object.keys(monthSet).forEach(function(mKey) {
    ...
    // ログ反映前に必ずレギュラー枠の背景を確定（振セルは applyBackground 内で黄を維持）
    applyBackground(sheet, year, month);
...
function updateSheetFromLogMap_(sheet, year, month, latestMap) {
  ...
  Object.keys(latestMap).forEach(function(key) {
    ...
    // 非レギュラー日は基本スキップ。ただし振替出席は非レギュラー日でも書き込む
    var isRegularDay = arrayContainsWeekday_(stInfo.regularDays, dateInfo.day);
    if (!isRegularDay && entry.status !== ROSTER.STATUS_DISPLAY['振替出席']) return;

    if (!updates[stInfo.row]) updates[stInfo.row] = {};
    updates[stInfo.row][dateInfo.col] = entry.status;
  });

  // 一括書き込み（値 + 背景色 + 文字色 + フォントサイズ）
  Object.keys(updates).forEach(function(rowStr) {
    var row = Number(rowStr);
    Object.keys(updates[rowStr]).forEach(function(colStr) {
      // updates には既に表示値（'振','欠','○','休'）が入っている
      var displayVal = updates[rowStr][colStr];
      var bg, fontColor, fontSize;

      if (displayVal === ROSTER.STATUS_DISPLAY['振替出席']) {  // '振'
        bg        = ROSTER.COLOR.LIGHT_YELLOW;
        fontColor = '#000000';
        fontSize  = 10;
      } else if (displayVal === ROSTER.STATUS_DISPLAY['休会']) { // '休'
        bg        = ROSTER.COLOR.GRAY;
        fontColor = '#666666';
        fontSize  = 10;
      } else if (displayVal === ROSTER.STATUS_DISPLAY['欠席']) { // '欠'
        bg        = ROSTER.COLOR.WHITE;
        fontColor = '#cc0000'; // 赤文字
        fontSize  = 10;
      } else { // '○'（出席）
        bg        = ROSTER.COLOR.WHITE;
        fontColor = '#000000';
...
function updateSummaryForSheet_(sheet, year, month) {
  ...
  for (var s = 0; s < numRows; s++) {
    ...
    for (var d = 0; d < numCols; d++) {
      ...
      if (!isRegular2) {
        // 非レギュラー日でも「振」は振替使用としてカウントする
        if (val === '振') makeup++;
        continue;
      }

      if (val === '欠') absent++;
      if (val === '振') makeup++;  // レギュラー日に「振替出席」と記録された場合も対応
...
function checkMissing() {
  ...
  for (var s = 0; s < numRows; s++) {
    ...
    for (var d = 0; d < numCols; d++) {
      ...
      if (val === '振') {
        // 振替出席（非レギュラー日含む）→ 薄黄を最優先（レギュラー外グレーに上書きしない）
 ...
 * @param {{rebuildLayout?: boolean}=} opt 既定 true（メニューと同じ）。日次バッチは false
 */
...
/**
 * @deprecated 代わりに installScheduledAutomationTriggers_（Automation.js）を使用
 */
function installRosterTriggers_() {
  installScheduledAutomationTriggers_();
}
